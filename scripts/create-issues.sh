...
echo "🚀 Creating Zakaty platform rewrite issues on $REPO …"
echo ""

# ---------------------------------------------------------------------------
# Helper: create_issue <title> <body> <labels>
# ---------------------------------------------------------------------------
create_issue() {
  local title="$1"
  local body="$2"
...
ensure_label "phase:1-foundation"    "0075ca" "Phase 1 – Foundation"
ensure_label "phase:2-core"          "e4e669" "Phase 2 – Core Features"
ensure_label "phase:3-saas"          "d93f0b" "Phase 3 – SaaS Layer"
ensure_label "phase:4-polish"        "bfd4f2" "Phase 4 – Polish & Localisation"
ensure_label "phase:5-testing"       "0e8a16" "Phase 5 – Testing & QA"
ensure_label "phase:6-deploy"        "5319e7" "Phase 6 – Deployment"
ensure_label "app:mobile"            "1d76db" "Flutter mobile app"
ensure_label "app:web"               "f9d0c4" "React web platform"
ensure_label "app:api"               "c5def5" "ASP.NET Core API"
ensure_label "app:infra"             "ededed" "Infrastructure / DevOps"
...
create_issue \
  "[Phase 1] Scaffold Flutter mobile app" \
  "## Overview
...
- All screen routes are registered (even if they show placeholder widgets)" \
  "phase:1-foundation,app:mobile,enhancement"

create_issue \
  "[Phase 1] Scaffold React web app" \
  "## Overview
...
- All routes return placeholder pages" \
  "phase:1-foundation,app:web,enhancement"

create_issue \
  "[Phase 1] Scaffold ASP.NET Core 8 API" \
  "## Overview
...
- Health-check endpoint returns 200" \
  "phase:1-foundation,app:api,enhancement"

create_issue \
  "[Phase 1] Set up PostgreSQL schema with EF Core migrations" \
  "## Overview
...
create_issue \
  "[Phase 1] Implement JWT authentication (register / login / refresh)" \
  "## Overview
...
create_issue \
  "[Phase 1] Port core Zakat business logic (Dart + C#)" \
  "## Overview
...
- 100% unit test coverage of calculation logic" \
  "phase:1-foundation,app:mobile,app:api,enhancement"
...
create_issue \
  "[Phase 2] Flutter: Zakat calculation screens (all 5 types)" \
  "## Overview
...
- Accessible with screen reader support" \
  "phase:2-core,app:mobile,enhancement"

create_issue \
  "[Phase 2] Flutter: Dashboard and calculation history screen" \
  "## Overview
...
create_issue \
  "[Phase 2] React: Public anonymous Zakat calculator page" \
  "## Overview
...
- Page scores ≥ 90 on Lighthouse (Performance + SEO)" \
  "phase:2-core,app:web,enhancement"

create_issue \
  "[Phase 2] React: Authenticated user dashboard" \
  "## Overview
...
create_issue \
  "[Phase 2] API: Public and private Zakat calculation endpoints" \
  "## Overview
...
- Private endpoint saves to \`ZakatCalculations\` table" \
  "phase:2-core,app:api,enhancement"

create_issue \
  "[Phase 2] API: Gold and silver price background job (Hangfire)" \
  "## Overview
...
create_issue \
  "[Phase 2] API: Calculation history CRUD endpoints" \
  "## Overview
...
- Currency and language preference applied globally" \
  "phase:3-saas,app:mobile,app:web,app:api,enhancement"

create_issue \
  "[Phase 3] API key generation and management" \
  "## Overview
...
- Keys are stored as bcrypt hashes (never plaintext)" \
  "phase:3-saas,app:web,app:api,enhancement"

create_issue \
  "[Phase 3] Subscription plan gating (free tier enforcement)" \
  "## Overview
...
create_issue \
  "[Phase 3] React: Pricing page and upgrade flow" \
  "## Overview
...
- Subscription status reflected immediately after upgrade" \
  "phase:3-saas,app:web,enhancement"

create_issue \
  "[Phase 3] Flutter: Premium feature gates (soft paywall)" \
  "## Overview
...
- Existing premium users bypass gate immediately" \
  "phase:3-saas,app:mobile,enhancement"

create_issue \
  "[Phase 3] API: Rate limiting middleware" \
  "## Overview
...
- Redis used as the distributed rate-limit store" \
  "phase:3-saas,app:api,enhancement"

# ===========================================================================
# PHASE 4 — POLISH & LOCALISATION (Weeks 11–13)
# ===========================================================================
echo "🌍 Phase 4 — Polish & Localisation …"

create_issue \
  "[Phase 4] Arabic (RTL) + English localisation — Flutter" \
  "## Overview
...
- All strings translated (no hardcoded English text)" \
  "phase:4-polish,app:mobile,enhancement"

create_issue \
  "[Phase 4] Arabic + English localisation — React web" \
  "## Overview
...
- Language preference persisted in \`localStorage\`" \
  "phase:4-polish,app:web,enhancement"

create_issue \
  "[Phase 4] PDF export — Flutter and React" \
  "## Overview
...
- PDF generates in < 2 seconds on mobile" \
  "phase:4-polish,app:mobile,app:web,enhancement"

create_issue \
  "[Phase 4] Dark mode support — Flutter" \
  "## Overview
...
create_issue \
  "[Phase 4] Responsive design — React web" \
  "## Overview
...
create_issue \
  "[Phase 4] Flutter: Onboarding flow (3-step intro)" \
  "## Overview
...
create_issue \
  "[Phase 4] React: Marketing landing page" \
  "## Overview
...
create_issue \
  "[Phase 5] Flutter: Unit and widget tests" \
  "## Overview
...
- Coverage report generated via \`flutter test --coverage\`" \
  "phase:5-testing,app:mobile,enhancement"

create_issue \
  "[Phase 5] React: Unit tests (Vitest + React Testing Library)" \
  "## Overview
...
- Coverage report available via \`npm run test:coverage\`" \
  "phase:5-testing,app:web,enhancement"

create_issue \
  "[Phase 5] API: Integration tests (WebApplicationFactory)" \
  "## Overview
...
- Tests use an in-memory or test PostgreSQL database" \
  "phase:5-testing,app:api,enhancement"

create_issue \
  "[Phase 5] Business logic test coverage ≥ 90%" \
  "## Overview
...
- Dart and C# implementations return identical results for a shared test matrix" \
  "phase:5-testing,app:mobile,app:api,enhancement"

create_issue \
  "[Phase 5] End-to-end tests (Playwright — web)" \
  "## Overview
...
- Container starts and passes health check within 30 seconds" \
  "phase:6-deploy,app:api,app:infra,enhancement"

create_issue \
  "[Phase 6] Infrastructure as Code (Terraform / Azure Bicep)" \
  "## Overview
...
- Parameters file for each environment (dev, prod)" \
  "phase:6-deploy,app:infra,enhancement"

create_issue \
  "[Phase 6] GitHub Actions CI/CD pipelines" \
  "## Overview
...
create_issue \
  "[Phase 6] Deploy API to Azure Container Apps" \
  "## Overview
...
create_issue \
  "[Phase 6] Deploy React to Azure Static Web Apps" \
  "## Overview
...
- Lighthouse Performance ≥ 90 in production" \
  "phase:6-deploy,app:web,app:infra,enhancement"

create_issue \
  "[Phase 6] Submit Flutter app to Google Play (beta)" \
  "## Overview
...
- CI/CD deploys new builds to internal track automatically on version tag" \
  "phase:6-deploy,app:mobile,app:infra,enhancement"

create_issue \
  "[Phase 6] Submit Flutter app to Apple App Store (beta via TestFlight)" \
  "## Overview
